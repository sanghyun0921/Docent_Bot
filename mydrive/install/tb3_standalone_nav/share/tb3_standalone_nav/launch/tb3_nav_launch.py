...
def generate_launch_description():
    ...
    controller_node = Node(
        package='tb3_standalone_nav',
        executable='dwa_controller_server',
        name='dwa_controller_server',
        output='screen',
        parameters=[{'use_sim_time': use_sim_time}]
    )

    # 🚨 Lifecycle 관리 대상 노드 이름 (매니저에게 파라미터로 전달됨)
    lifecycle_nodes =['map_server', 'amcl_node', 'a_star_planner_server', 'dwa_controller_server']
    ...
    # ⏱️ 핵심: 매니저가 서버들을 모두 Active 상태로 만들 때까지 7초간 기다렸다가 bt_navigator 실행!
    ...
    return LaunchDescription([
        ...
        map_server_node,
        ekf_node, # EKF는 통상 일반 노드로 동작하므로 lifecycle_nodes 명단에서 뺐습니다.
        amcl_node,
        planner_node,
        controller_node,
        lifecycle_manager_node,
        delayed_bt_navigator  # 지연 실행되는 BT Navigator 추가!
    ])
